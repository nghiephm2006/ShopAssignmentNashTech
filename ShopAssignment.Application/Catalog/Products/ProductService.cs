...
using ShopAssignment.Utilities.Exceptions;
...
namespace ShopAssignment.Application.Catalog.Products
{
    public class ProductService : IProductService
    {
        ...
        public ProductService(ShopAssignmentDBContext context, IStorageService storageService)
        {
            _context = context;
        ...
        {
            var product = await _context.Products.FindAsync(productId);
            if (product == null) throw new ShopAssignmentException($"Cannot find a product: {productId}");
        ...
        public async Task<ProductImageViewModel> GetImageById(int imageId)
        {
            ...
            if (image == null)
                throw new ShopAssignmentException($"Cannot find an image with id {imageId}");
        ...
        public async Task<int> Update(ProductUpdateRequest request)
        {
            ...
            if (product == null || productTranslations == null) throw new ShopAssignmentException($"Cannot find a product with id: {request.Id}");
        ...
        public async Task<bool> UpdatePriceNStock(int productId, PriceAndStockUpdateRequest request)
        {
            var product = await _context.Products.FindAsync(productId);
            if (product == null) throw new ShopAssignmentException($"Cannot find a product with id: {productId}");
            if (request.NewPrice == 0)
            {
                ...
                product.Stock += request.AddedQuantity;
            }
            else
            {
                if (request.AddedQuantity == 0)
